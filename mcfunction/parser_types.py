...
class Block(ParserType):
    ...
    def parse(self, parts: t.Iterator[str]) -> nodes.BlockNode:
        ...
        if match is None:
            raise ParserException(f"expected valid block, not {arg!r}")
        is_tag, namespace, name, blockstates, datatags = match.groups()
        if namespace is not None:
            namespace = namespace[:-1]  # remove ':'
        return nodes.BlockNode(bool(is_tag), namespace, name, blockstates, datatags)
...
class Entity(ParserType):
    ...
    def parse(self, parts: t.Iterator[str]) -> nodes.EntityNode:
        ...
        if selector.startswith("@"):
            ...
            if not self.valid_usernames.fullmatch(selector) and not UUID.uuid.fullmatch(
                selector
            ):
...
class Function(ParserType):
    ...
    def parse(self, parts: t.Iterator[str]) -> nodes.FunctionNode:
        arg = get(parts)
        match = self.namespace.fullmatch(arg)
        if match is None:
            raise ParserException(f"expected valid function, not {arg!r}")
...
class Item(ParserType):
    ...
    def parse(self, parts: t.Iterator[str]) -> nodes.ItemNode:
        ...
        match = self.namespace.fullmatch(arg)
        if match is None:
            raise ParserException(f"expected valid item, not {arg!r}")
...
class NamespaceID(ParserType):
    ...
    def parse(self, parts: t.Iterator[str]) -> nodes.NamespaceIDNode:
        arg = get(parts)
        match = self.namespace.fullmatch(arg)
        if match is None:
            raise ParserException(f"expected valid namespace identifier, not {arg!r}")
...
class Particle(ParserType):
    ...
    def parse(self, parts: t.Iterator[str]) -> nodes.ParticleNode:
        ...
        match = self.namespace.fullmatch(arg)
        if match is None:
            raise ParserException(f"expected valid particle, not {arg!r}")
        namespace, name = match.groups()
        if namespace is not None:
            namespace = namespace[:-1]  # remove ':'
        ...
        if namespace is None or namespace == "minecraft":
            if name == "dust":
                arguments = tuple(nodes.DoubleNode(float(x)) for x in get(parts, 4))
...
class Position(ParserType):
    ...
    def parse(self, parts: t.Iterator[str]) -> nodes.PositionNode:
        coordinates = self.split_coordinates(parts)
        return nodes.PositionNode(
            Coordinate.parse(coordinates),
            Coordinate.parse(coordinates),
            Coordinate.parse(coordinates),
        )
...
class Position2d(ParserType):
    def parse(self, parts: t.Iterator[str]) -> nodes.Position2dNode:
        return nodes.Position2dNode(Coordinate.parse(parts), Coordinate.parse(parts))


class Rotation(ParserType):
    def parse(self, parts: t.Iterator[str]) -> nodes.RotationNode:
        return nodes.RotationNode(Coordinate.parse(parts), Coordinate.parse(parts))
...
class Union(ParserType):
    ...
    def parse(self, parts: t.Iterator[str]) -> nodes.RawNode:
        ...
        if arg not in self._options:
            raise ParserException(f"expected any of {self._options!r}, not {arg!r}")
        return nodes.RawNode(arg)
