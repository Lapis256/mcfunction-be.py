...
from ...parser_types import Block, Entity, Integer, Literal, Position, Any, Union
...
class ParsedExecuteCommand(ParsedCommand):
    ...
    run: Union(ParsedCommand, Any)  # ParsedCommand
    ...
    def __str__(self):
        ...
        if self.detect is not None:
            ...
            return (
                f"{base} {self.detect} {self.detect_position} "
    ...
    def parse(self, command: str, version: MinecraftVersion = None):
        ...
        for variation in self.variations:
            ...
            for no, parser in enumerate(variation):
                ...
                # result["run"] = " ".join(
                #     parts
                # )  # parse_command(" ".join(parts), version)
                result["run"] = " ".join(parts)
                try:
                    result["run"] = parse_command(result["run"], version)
                except:
                    pass
...
execute.add_variation(Parser(Entity(), "target"), Parser(Position(), "position"))
